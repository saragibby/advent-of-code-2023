...
class Hand
  ...
  def type
    ...
    non_jokers = card_counts.reject{ |k, v| k == "J" }

    case
    when non_jokers.select{ |k, v| v == 5 }.count == 1, jokers == 5, jokers > 0 && non_jokers.count == 1
      HAND_ORDER[6]
    when non_jokers.select{ |k, v| v == 4 }.count == 1, jokers == 4, non_jokers.max_by { | key, value | value }[1] + jokers == 4
      HAND_ORDER[5]
    when non_jokers.select{ |k, v| v == 3 }.count == 1 && non_jokers.select{ |k, v| v == 2 }.count == 1,
    non_jokers.select{ |k, v| v == 2 }.count == 2 && jokers == 1
      HAND_ORDER[4]
    when non_jokers.select{ |k, v| v == 3 }.count == 1, jokers == 2, non_jokers.select{ |k, v| v == 2 }.count == 1 && jokers == 1
      HAND_ORDER[3]
    when non_jokers.select{ |k, v| v == 2 }.count == 2, non_jokers.select{ |k, v| v == 2 }.count == 1 && jokers == 1
      HAND_ORDER[2]
    when non_jokers.select{ |k, v| v == 2 }.count == 1, jokers == 1
      HAND_ORDER[1]
    else
      HAND_ORDER[0]
    end
